...
const DuplicateModal: React.FC<{ 
    ...
    const handleDuplicateAction = async () => {
        ...
        try {
            ...
            let finalThankYou = { ...fullPage.thank_you_content };

            if (strategy === 'reword') {
                setProgress(`Rielaborazione testi tono ${targetTone} con AI...`);
                finalContent = await rewriteLandingPage(fullPage.content, targetTone);
                if (fullPage.thank_you_content) {
                  finalThankYou = await rewriteLandingPage(fullPage.thank_you_content, targetTone);
                } else {
        ...
        <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4">
            ...
            <div className="relative bg-white rounded-[2.5rem] shadow-2xl w-full max-w-xl overflow-hidden animate-in zoom-in-95 duration-300 border border-white/20">
                ...
                <div className="bg-gradient-to-br from-slate-900 to-slate-800 p-8 text-white relative">
                    ...
                    <p className="text-slate-400 text-sm font-medium">Clona o reinventa la tua pagina in un click.</p>
                </div>
...
const PageCard = React.memo(({ page, onView, onEdit, onDuplicate, onDownload, onDelete }: { 
    page: LandingPageRow, onView: (p: LandingPageRow) => void, onEdit?: (p: LandingPageRow) => void, 
    onDuplicate?: (p: LandingPageRow) => void, onDownload?: (p: LandingPageRow) => void, onDelete?: (id: string) => void
}) => (
    <div className="group bg-white rounded-2xl shadow-sm hover:shadow-xl border border-slate-100 overflow-hidden transition-all duration-300 cursor-pointer hover:-translate-y-1 relative" onClick={() => onView(page)}>
         <div className="absolute top-0 right-0 p-2 opacity-0 group-hover:opacity-100 transition-opacity flex gap-1 bg-slate-900/80 rounded-bl-xl backdrop-blur z-20" onClick={(e) => e.stopPropagation()}>
            {onDownload && <button onClick={(e) => { e.stopPropagation(); onDownload(page); }} className="p-2 hover:bg-blue-500 rounded-lg text-white transition-colors" title="Scarica (JSON)"><Download className="w-4 h-4"/></button>}
...
export const App: React.FC = () => {
  ...
  const handleDownloadPage = async (page: LandingPageRow) => {
    let fullPage = page;
    ...
    const dataStr = JSON.stringify(fullPage, null, 2);
    const dataUri = 'data:application/json;charset=utf-8,'+ encodeURIComponent(dataStr);
    
    const exportFileDefaultName = `${formatSlug(fullPage.product_name)}-landing.json`;
  ...
  const handleSaveToDb = async (asNew = false) => {
    ...
    try {
        ...
        const newTySlug = asNew ? (newSlug + '-grazie') : tySlug;
  ...
  const handleEdit = async (page: LandingPageRow) => {
    ...
    setProduct({
        ...
        targetAudience: '', tone: PageTone.PROFESSIONAL, language: 'Italiano',
    ...
    setGeneratedThankYouContent(fullPage.thank_you_content || createDefaultThankYouContent(fullPage.content));
                    ...
                    {!generatedContent ? (
                                ...
                                <div className="space-y-3 p-3 bg-slate-50 rounded-2xl border border-slate-100">
                                    ...
                                    <div className="grid grid-cols-3 gap-2">
                                        ...
                                        <div>
                                            ...
                                            <input type="number" min="0" max="2000" value={reviewCount} onChange={e => setReviewCount(parseInt(e.target.value) || 0)} className="w-full border border-slate-200 rounded-xl p-2.5 text-xs focus:ring-2 focus:ring-emerald-500 outline-none transition-all text-center" />
                                        </div>
                                        <div>
                        ...
                        <div className="space-y-4 max-h-[85vh] overflow-y-auto pr-2 custom-scrollbar">
                            ...
                            <div className="px-1 mb-4 flex gap-2">
                                <button 
                                    onClick={() => handleDownloadPage({ id: editingPageId || '', product_name: product.name, content: generatedContent!, thank_you_content: generatedThankYouContent!, slug, thank_you_slug: tySlug, niche: product.niche, is_published: isPublished, created_at: new Date().toISOString() })}
                                    className="flex-1 bg-white border border-slate-200 text-slate-600 py-2 px-3 rounded-xl text-xs font-bold flex items-center justify-center gap-2 hover:bg-slate-50 transition-all shadow-sm"
                                >
                                    <Download className="w-4 h-4" /> Esporta Configurazione
                                </button>
                            </div>
                                                ...
                                                <input 
                                                    ...
                                                    onChange={e => {
                                                        ...
                                                        if (isEnabling && vids.length === 0) {
                                                            ...
                                                            if (!title) title = defaultData.title || "Guarda i nostri video";
                                                        }
                                            ...
                                            {generatedContent.boxContent?.enabled && (
                                                <div className="space-y-4 animate-in fade-in">
                                                    ...
                                                    <div className="space-y-2">
                                                        ...
                                                        <button 
                                                            onClick={() => updateContent({ boxContent: { ...generatedContent.boxContent!, items: [...generatedContent.boxContent!.items, "Nuovo elemento"] } })} 
                        ...
                        <div className="space-y-6">
                            ...
                            {isLoadingPages ? (
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-in fade-in duration-700">
                                    {adminPages.map(page => <PageCard key={page.id} page={page} onView={handleViewPage} onEdit={handleEdit} onDuplicate={handleOpenDuplicateModal} onDownload={handleDownloadPage} onDelete={handleDelete} />)}
                                </div>
                            )}
                        </div>
                    ) : (
